...
class proyectil:#separacion en clases, procesar por separado las fisicas y los graficos, luego se unen con una clase principal
    def __init__(self,screen,x,y,width,height,masa,color,img):
        ...
        proyectiles.append(self)
        active_proyectil = self
    def draw(self):
        if lanzar.boton_state:
            pos = self.fisicas.actualizar()
            if self.img:
                self.graficos.dibujarConImagen(self.screen, pos, self.tamaño, self.img)
            else:
                self.graficos.dibujar(self.screen, pos, self.tamaño, self.color)
...
class grproyectil:
    ...
    def dibujarTrayectoria(self, screen, posicion):
        ...
        for tx, ty in self.trayectoria:
            pygame.draw.rect(screen, (250, 0, 0), (tx, ty, 2, 2))
...
proyectilv3 = proyectil(screen, 0, 45, 4, 4, 1, (250, 0, 0),imgnave)
...
lanzar = widgets.boton(screen,1, 1, 150, 50, (0, 250, 0),(250, 0, 0), "Lanzar","Parar", (255, 255, 255),reiniciar,lanzarActivo)
